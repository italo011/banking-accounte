...
def is_valid_mobile(mobile_number):
    if mobile_number.__len__() == 12 and mobile_number.isnumeric():
        return True
    else:
        return False
...
def display_account_summary(identity, choice):  
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            if choice == 1:
                output_message += "Numero da conta : " + line.replace("\n", "") + "\n"
                customer_database.__next__()  
                output_message += "Saldo atual : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Data de criação da conta : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Nome do titular da conta : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Tipo de conta : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Data de nascimento : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Numero do telefone : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Gênero : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "Nacionalidade : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "cpf : " + customer_database.__next__().replace("\n", "") + "\n"
            else:
                customer_database.readline()  
                output_message += "Saldo: " + customer_database.readline().replace("\n", "") + "\n"
            flag = 1
            break
            ...
            for index in range(11):
                ...
                if fetched_line is not None:
                    ...
                    break
    if flag == 0:
        print("\n# Nenhuma conta associada ao numero da conta! #")
...
def delete_customer_account(identity, choice):  
    ...
    if flag == 1:
        output_message = "Conta com conta nº." + str(identity) + " Fechado com sucesso!"
        if choice == 1:
            adminMenu.printMessage_outside(output_message)
        print(output_message)
    else:
        output_message = "A conta não existe !"
        if choice == 1:
            adminMenu.printMessage_outside(output_message)
        print(output_message)
...
def create_admin_account(identity, password):
    ...
    output_message = "Conta de administrador criada com sucesso  !"
    adminMenu.printMessage_outside(output_message)
    print(output_message)
...
def delete_admin_account(identity):
    ...
    if flag == 1:
        output_message = "Conta com ID da conta " + identity + " Fechado com sucesso!"
        print(output_message)
        adminMenu.printMessage_outside(output_message)
    else:
        output_message = "A conta não existe :("
...
def change_PIN(identity, new_PIN):
    ...
    output_message = "Senha alterada com exito."
...
def check_credentials(identity, password, choice,
    ...
    try:
        ...
        print("#", folder_name[2:], "base de dados não existe!\n# New", folder_name[2:],
              "Base de dados criado com sucesso.")
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("Bem vindo ao banco atacante")
        ...
        self.Canvas1 = tk.Canvas(window, background="#ffffff", borderwidth="0", insertbackground="black",
                                 relief="ridge",
        ...
        self.Label1 = tk.Label(self.Canvas1, background="#808080", disabledforeground="#a3a3a3",
                               font="-family {Segoe UI} -size 13 -weight bold", foreground="#000000",
                               text='''Escolha sua seção''')
...
class adminLogin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Admin banco atacante")
        ...
        self.Label1 = tk.Label(Canvas1, background="black", disabledforeground="#a3a3a3",
        ...
        self.Button = tk.Button(Canvas1, text="Login", borderwidth="0", width=10, background="black",
                                foreground="#00254a",
                                font="-family {Segoe UI} -size 10 -weight bold",
                                command=lambda: self.login(self.Entry1.get(), self.Entry1_1.get()))
        self.Button.place(relx=0.765, rely=0.755)

        self.Button_back = tk.Button(Canvas1, text="Voltar", borderwidth="0", width=10, background="black",
                                     foreground="#00254a",
                                     font="-family {Segoe UI} -size 10 -weight bold",
                                     command=self.back)
    ...
    def login(self, admin_id, admin_password):
        ...
        if check_credentials(admin_id, admin_password, 1, True):
            ...
            Error.setMessage(self, message_shown="Credenciais invalidas!")
            self.setImg()
...
class CustomerLogin:
    def __init__(self, window=None):
        ...
        self.Button_back = tk.Button(Canvas1, text="Voltar", borderwidth="0", width=10, background="#00254a",
                                     foreground="#ffffff",
                                     font="-family {Segoe UI} -size 10 -weight bold",
                                     command=self.back)
        self.Button_back.place(relx=0.545, rely=0.755)
...
class adminMenu:
    def __init__(self, window=None):
        ...
        self.Labelframe1 = tk.LabelFrame(window, relief='groove', font="-family {Segoe UI} -size 13 -weight bold",
                                         foreground="#001c37", text="Selecione sua opção", background="#fffffe")
        ...
        self.Button1 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 text="Fechar esta conta", command=self.closeAccount)
        ...
        self.Button2 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="Criar conta no banco", command=self.createCustaccount)
        ...
        self.Button4 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="Criar conta admin", command=self.createAdmin)
        ...
        self.Button5 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="Fechar conta admin ", command=self.deleteAdmin)
        ...
        self.Button6 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="Verificar resumo da conta", command=self.showAccountSummary)
...
class CloseAccountByAdmin:
    def __init__(self, window=None):
        ...
        self.Button1 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", borderwidth="0",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text="Voltar",
                                 command=self.back)
        ...
        self.Button2 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text="Prosseguir",
    ...
    def submit(self, identity):
        if not is_valid(identity):
            ...
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="A conta não existe!")
            return
        self.master.withdraw()
...
class createCustomerAccount:
    def __init__(self, window=None):
        ...
        self.Label1 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               ...
                               highlightcolor="black", text='''Numero da conta:''')
        ...
        self.Label2 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''Nome:''')
        ...
        self.Label3 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''Tipo da conta:''')
        ...
        self.Radiobutton1 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           ...
                                           text='''Poupança''', variable=acc_type, value="Poupança")
        ...
        self.Radiobutton1_1 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                             background="#f2f3f4", disabledforeground="#a3a3a3", foreground="#000000",
                                             highlightbackground="#d9d9d9", highlightcolor="black", justify='left',
                                             text='''Corrente''', variable=acc_type, value="Corrente")
        ...
        self.Label5 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               ...
                               highlightcolor="black", text='''Numero de telefone:''')
        ...
        self.Label4 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000",
                               highlightcolor="black", text='''Data de nascimento:''')
        ...
        self.Label6 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000",
                               highlightcolor="black", text='''Gênero:''')
        ...
        self.Radiobutton3 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           ...
                                           text='''Masculino''', variable=gender, value="Masculino")
        ...
        self.Radiobutton4 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           background="#f2f3f4", disabledforeground="#a3a3a3", foreground="#000000",
                                           highlightbackground="#d9d9d9", highlightcolor="black", justify='left',
                                           text='''Feminino''', variable=gender, value="Feminino")
        ...
        self.Label7 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''Nacionalidade:''')
        ...
        self.Label10 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                                disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                                highlightcolor="black", text='''confirmasenha:''')
        ...
        self.Label11 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                                disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                                highlightcolor="black", text='''Saldo inicial:''')
    ...
    def create_acc(self, customer_account_number, name, account_type, date_of_birth, mobile_number, gender, nationality,
        ...
        if is_valid(customer_account_number) and customer_account_number.isnumeric():
            if name != "":
                if account_type == "Poupança" or account_type == "Corrente":
                    if check_date(date_of_birth):
                        if is_valid_mobile(mobile_number):
                            if gender == "Masculino" or gender == "Feminino":
                                if nationality.__len__() != 0:
                                    if KYC_document.__len__() != 0:
                                        if PIN.isnumeric() and PIN.__len__() == 4:
                                            if confirm_PIN == PIN:
                                                if initial_balance.isnumeric():
                                                    output_message = "Conta cliente criada com sucesso!"
                                                    print(output_message)
                                                    adminMenu.printMessage_outside(output_message)
                                                else:
                                                    Error(Toplevel(self.master))
                                                    Error.setMessage(self, message_shown="Saldo invalido!")
                                                    return
                                            else:
                                                Error(Toplevel(self.master))
                                                Error.setMessage(self, message_shown="Senha incompativel!")
                                                return
                                        else:
                                            Error(Toplevel(self.master))
                                            Error.setMessage(self, message_shown="Senha invalida!")
                                            return
                                    else:
                                        Error(Toplevel(self.master))
                                        Error.setMessage(self, message_shown="documento!")
                                        return
                                else:
                                    Error(Toplevel(self.master))
                                    Error.setMessage(self, message_shown="Nacionalidade!")
                                    return
                            else:
                                Error(Toplevel(self.master))
                                Error.setMessage(self, message_shown="Gênero!")
                                return
                        else:
                            Error(Toplevel(self.master))
                            Error.setMessage(self, message_shown="Numero de telefone invalido!")
                            return
                    else:
                        Error(Toplevel(self.master))
                        Error.setMessage(self, message_shown="Data invalida!")
                        return
                else:
                    Error(Toplevel(self.master))
                    Error.setMessage(self, message_shown="tipo de conta!")
                    return
            else:
                Error(Toplevel(self.master))
                Error.setMessage(self, message_shown="O nome não pode ficar vazio!")
                return
        else:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="Numero da acc é invalido!")
            return
...
class createAdmin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Criar conta admin")
        window.configure(background="#f2f3f4")
        ...
        self.Label3 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''Confirmar senha:''')
    ...
    def create_admin_account(self, identity, password, confirm_password):
        if check_credentials(identity, "DO_NOT_CHECK_ADMIN", 1, False):
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="O código não está disponível!")
        else:
            if password == confirm_password and len(password) != 0:
                ...
                if password != confirm_password:
                    Error.setMessage(self, message_shown="Senha errada!")
                else:
                    Error.setMessage(self, message_shown="Senha invalida!")
...
class deleteAdmin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Excluir conta admin")
        window.configure(background="#f2f3f4")
    ...
    def delete_admin(self, admin_id, password):
        if admin_id == "aayush" or admin_id == admin_idNO:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="Operação negada!")
            return
...
class customerMenu:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Seção cliente")
        window.configure(background="#00254a")

        self.Labelframe1 = tk.LabelFrame(window, relief='groove', font="-family {Segoe UI} -size 13 -weight bold",
                                         foreground="#000000", text='''Selecione sua opção''', background="#fffffe")
        self.Labelframe1.place(relx=0.081, rely=0.081, relheight=0.415, relwidth=0.848)
        ...
        self.Button1 = tk.Button(self.Labelframe1, command=self.selectWithdraw, activebackground="#ececec",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text='''Retirar''')
        self.Button1.place(relx=0.667, rely=0.195, height=34, width=181, bordermode='ignore')

        self.Button2 = tk.Button(self.Labelframe1, command=self.selectDeposit, activebackground="#ececec",
                                 activeforeground="#000000", background="#39a9fc", borderwidth="0",
                                 disabledforeground="#a3a3a3", font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text='''Depositar''')
        self.Button2.place(relx=0.04, rely=0.195, height=34, width=181, bordermode='ignore')
        ...
        self.Button4 = tk.Button(self.Labelframe1, command=self.selectChangePIN, activebackground="#ececec",
                                 activeforeground="#000000", background="#39a9fc", borderwidth="0",
                                 disabledforeground="#a3a3a3", font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text='''Mudar Senha''')
        self.Button4.place(relx=0.04, rely=0.439, height=34, width=181, bordermode='ignore')
        ...
        self.Button6 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text='''Checar seu saldo''', command=self.checkBalance)
        self.Button6.place(relx=0.04, rely=0.683, height=34, width=181, bordermode='ignore')
    ...
    def checkBalance(self):
        ...
        print("função de verificação de saldo chamada.")
...
class depositMoney:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Deposite dinheiro")
        ...
        self.Label1 = tk.Label(window, background="#f2f3f4", disabledforeground="#a3a3a3",
                               ...
                               text='''Insira o valor a depositar :''')
        ...
        self.Button1 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 ...
                                 highlightcolor="black", pady="0", text='''Prosseguir''',
                                 command=lambda: self.submit(self.Entry1.get()))
        ...
        if amount.isnumeric():
            if 25000 >= float(amount) > 0:
                ...
                if float(amount) > 25000:
                    Error.setMessage(self, message_shown="Limite excedido!")
                else:
                    Error.setMessage(self, message_shown="Valor positivo esperado!")
                return
        else:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="Dinheiro invalido!")
            return
        if output == -1:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="Falha na transação!")
            return
        else:
            output = "Quantidade de Reais " + str(amount) + " depositado com sucesso.\nAtualize saldo : " + str(output)
...
class withdrawMoney:
    def __init__(self, window=None):
        ...
        self.Label1 = tk.Label(window, background="#f2f3f4", disabledforeground="#a3a3a3",
                               ...
                               text='''Insira o valor para sacar :''')
        self.Label1.place(relx=0.146, rely=0.171, height=21, width=164)
    ...
    def submit(self, amount):
        ...
        if output == -1:
            ...
            output = "Quantidade em Reais " + str(amount) + " Saque efetuado com sucesso.\nAtualize saldo : " + str(output)
            customerMenu.printMessage_outside(output)
            self.master.withdraw()
...
class changePIN:
    def __init__(self, window=None):
        ...
        self.Button2 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 disabledforeground="#a3a3a3", foreground="#ffffff", borderwidth="0",
                                 highlightbackground="#d9d9d9",
                                 highlightcolor="black", pady="0", text="Voltar", command=self.back)
    ...
    def submit(self, new_PIN, confirm_new_PIN):
        if new_PIN == confirm_new_PIN and str(new_PIN).__len__() == 4 and new_PIN.isnumeric():
            ...
            if new_PIN != confirm_new_PIN:
                ...
                Error.setMessage(self, message_shown="Senha deve conter 4 caracteres!")
            else:
                Error.setMessage(self, message_shown="Senha invalida!")
            return
...
class checkAccountSummary:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("Verificar resumo da conta")
        window.configure(background="#f2f3f4")
    ...
    def submit(self, identity):
        if not is_valid(identity):
            ...
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="Id não existe!")
            return
        self.master.withdraw()
